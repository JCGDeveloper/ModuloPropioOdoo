...
class Paquete(models.Model):
    ...
    # Campo computado con la fecha de la última actualización
    ...
    def _compute_last_update(self):
        for record in self:
            if record.actualizaciones_ids:
                # Obtener la mayor (más reciente) fecha
                record.last_update = max(record.actualizaciones_ids.mapped('fecha'))
            else:
